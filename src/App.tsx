      ...
      <div className={`inline-flex items-center rounded-full bg-white/90 backdrop-blur-md px-2 py-2 transition-all w-full md:w-auto overflow-x-auto no-scrollbar ${scrolled ? 'shadow-lg shadow-black/5 border border-stroke' : 'border border-stroke/50'}`}>
        ...
        <div className="flex items-center space-x-1 shrink-0">
          {["Início", "Sobre", "Diferenciais", "Depoimentos"].map((link, i) => (
            <a key={link} href={`#${link.toLowerCase()}`} className={`text-xs sm:text-sm rounded-full px-3 py-1.5 transition-colors ${i===0 ? 'text-accent bg-blue-50 font-medium' : 'text-gray-600 hover:text-accent hover:bg-blue-50/50'}`}>
              {link}
            </a>
          ))}
        </div>
    ...
    <section id="início" className="relative w-full min-h-[100svh] flex flex-col justify-center overflow-hidden bg-white">
      <div className="absolute inset-0 z-0 hero-reveal">
        <img 
          ...
          alt="Dra Edilene" 
          className="w-full h-full object-cover object-[50%_20%] scale-[0.99]" 
        />
